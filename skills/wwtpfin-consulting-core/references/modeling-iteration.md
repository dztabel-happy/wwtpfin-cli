...
Read the complete constraints, fixed-parameter changes, comparability and conditions, not only `shortlist`. `numerically_feasible` covers numerical and fixed-boundary checks; `feasible` also requires comparable cost/payment scope. `selectable_shortlist` additionally requires recorded implementation conditions. A successful command or complete deliverable is not project approval. Empty selectable results require further iteration or condition resolution.
