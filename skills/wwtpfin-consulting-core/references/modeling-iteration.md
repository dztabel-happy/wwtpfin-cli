...
Read the complete constraints, fixed-parameter changes, comparability, `model_limits` and conditions, not only `shortlist`. `numerically_feasible` covers numerical and fixed-boundary checks; `feasible` also requires comparable cost/payment scope. `selectable_shortlist` additionally requires no unresolved model limits and recorded implementation conditions. `research_selectable_shortlist` permits a user to confirm research assumptions while implementation conditions remain pending; it still requires a bound contract, numerical/fixed-boundary and comparability checks, and no unresolved model limits. A successful command or complete deliverable is not project approval. An empty implementation shortlist can therefore still support an explicitly selected conditional study; an empty research shortlist requires further modeling or iteration.
...
Keep `decision_status=user_confirmation_required` until a choice is actually made. Only after explicit confirmation, record a selection using `scheme-selection`: exact comparison content hash, candidate ID, canonical parameter hash, confirmation identity/time/reference and purpose. Record the scope separately: `selection_scope=conditional_research` only when the user explicitly chooses to complete the study under its stated assumptions; keep every pending implementation condition and failure fallback. `selection_scope=implementation` (the default for older records) retains the strict condition gate. Research selection never bypasses numerical, fixed-boundary, comparability or model limits. Use the bundled [selection-record helper](../scripts/record-selection.py), with `--selection-scope conditional_research` for that explicitly confirmed scope, to bind these values without transcribing hashes. Do not hand-copy rounded prices into the selected input.
...
`finalize` rechecks the contract, evidence, software, full parameters, numerical constraints and implementation-condition records under the confirmed selection scope, then runs the existing complete calculation/build chain. Both `CURRENT.md` and the resulting financial core retain the conditional-study scope, pending conditions and simulation label when applicable. Changed inputs, evidence, software or decisions require a new comparison and confirmation. Failed runs remain staging; successful runs are immutable.
