...
Run downstream tools only when the user requests a rendered chart, Word report, or finished document. Select the latest verified run according to [workspace management](workspace.md), or the exact verified run requested by the user.

Use this order: ChartKit first when figures are needed, then DocxKit. Both products consume the selected run; neither recalculates financial results.
...
Read the selected run's `deliverable/document.json` as the authoritative report semantics, with `report.md`, tables, source references and verified ChartKit assets as supporting inputs. Convert them into DocxKit's editable source or `report.json`, then invoke the installed `docxkit` skill under `exports/run-NNNN/docx/`.
