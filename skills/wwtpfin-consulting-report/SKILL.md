...
2. Bind all work to `exports/run-NNNN/report/`. Both plan initialization and checking invoke the CLI to verify the source package and its current case selection. Default purpose is `current`; explicitly choose `historical`, `baseline`, or `conditional_research` for those reports. A baseline or conditional report inside a case must still match its current registered output; use `historical` for an outdated result. Standalone conditional reports must bind an actual selected conditional-research run. The plan records source identity, hashes and a required Chinese notice. Never mix runs.
...
- `--mode draft/final` describes content completeness only. A complete baseline, historical or conditional report can use `final`; this never changes selection scope, settles pending conditions, or establishes implementation approval.
...
```bash
python scripts/init_report_plan.py RUN/deliverable EXPORT/report-plan.json --source-run run-NNNN --case CASE
...
# 明确的独立用途：初始化时追加 --purpose historical、baseline 或 conditional_research
# 冻结环境：init 和 check 均可追加 --cli /absolute/path/to/matching/wwtp-fin
```
