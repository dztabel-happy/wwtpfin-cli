...
| 3 选定与复核 | 用户明确选择、对应比较、完整参数、证据与条件记录 | 选定方案全量重算、检查和绑定成果；剩余边界 | 选择必须绑定当前比较；实施条件未满足不能改成已满足；新事实影响结果时返回前阶段 |
...
收到用户对当前候选的明确选择且计算依据未改变时，将选择记录保存到新的 `selections/selection-NNNN.json`。记录“收到选择、正在完整重算”这一轮时，仍须显式绑定当前比较，不能先用无产物轮次撤掉它，再尝试定稿：
